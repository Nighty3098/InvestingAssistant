...
class DatabaseManager:
    def __init__(self):
        ...
        self.db_params = {
            ...
            "port": os.getenv("POSTGRES_PORT", "999"),
        }
        self._init_database()
    ...
    def get_connection(self):
        ...
        try:
            ...
            logger.debug(
                f"Connected to PostgreSQL DB at {self.db_params['host']}:{self.db_params['port']}"
            )
            yield connection
    ...
    def _recreate_stocks_table(self):
        self.execute_query("DROP TABLE IF EXISTS stocks CASCADE")
        self._create_stocks_table()

    def _init_database(self):
        self._create_roles_table()
        self._create_users_table()
        # self._recreate_stocks_table()
            ...
            try:
                ...
                try:
                    ...
                    if stock_info is None or not isinstance(stock_info, dict):
                        logger.warning(
                            f"Invalid stock info received for {ticker}: {stock_info}"
                        )
                        return ticker, "Error"
    ...
    def check_user_ban(self, username: str) -> bool:
        result = self.execute_query(
            "SELECT is_banned FROM users WHERE username = %s", (username,), fetch=True
        )
    ...
    def get_user_stocks(self, user_id: int) -> Dict[str, float]:
        stocks = self.execute_query(
            "SELECT stock_name FROM stocks WHERE user_id = %s", (user_id,), fetch=True
        )
    ...
    def _format_stock_info_dict(self, stock_info, recommendations):
        return {
            ...
            "dividend_yield": stock_info.get(
                "dividendYield", "Dividend yield not found"
            ),
            "recommendations": recommendations,
            "target_mean_price": stock_info.get(
                "targetMeanPrice", "Target mean price not found"
            ),
            "target_high_price": stock_info.get(
                "targetHighPrice", "Target high price not found"
            ),
            "target_low_price": stock_info.get(
                "targetLowPrice", "Target low price not found"
            ),
    ...
    def remove_account(self, user_id):
        ...
        try:
            self.execute_query("DELETE FROM stocks WHERE user_id = %s", (user_id,))
    ...
    def get_user_role(self, user_id):
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT role_id FROM users WHERE user_id = %s", (user_id,)
                )
                result = cursor.fetchone()

                if result is not None:
                    cursor.execute(
                        "SELECT role_name FROM roles WHERE id = %s", (result[0],)
                    )
    ...
    def get_users_list(self):
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT user_id, username, network_tokens, role_id FROM users"
                )
                result = cursor.fetchall()
                return result
        except Exception as e:
    ...
    def get_network_tokens(self, user_id):
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT network_tokens FROM users WHERE user_id = %s", (user_id,)
                )
                result = cursor.fetchone()
    ...
    def update_tokens(self, user_id, token_change):
        try:
            ...
            if sign not in ("+", "-"):
                raise ValueError(
                    "Invalid token change format. Use '+<number>' or '-<number>'."
                )

            update_query = """
                UPDATE users
                SET network_tokens = network_tokens {} %s
                WHERE user_id = %s
            """.format("+" if sign == "+" else "-")
    ...
    def get_stocks_list(self, user_id):
        with self.get_connection() as conn:
            try:
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT stock_name, quantity FROM stocks WHERE user_id=%s",
                    (user_id,),
                )
    ...
    def add_stock_to_db(self, user_id, stock_name, quantity):
        """Добавление акции в БД (increments quantity if stock already exists)"""
        with self.get_connection() as conn:
            try:
                cursor = conn.cursor()
                cursor.execute(
    ...
    def remove_stock_from_db(self, user_id, stock_name):
        ...
        with self.get_connection() as conn:
            try:
                ...
                cursor.execute(
                    """
                    DELETE FROM stocks WHERE user_id = %s AND stock_name = %s
                    """,
                    (user_id, stock_name),
                )
                conn.commit()
    ...
    def update_stock_quantity(self, user_id, stock_name, quantity):
        ...
        with self.get_connection() as conn:
            try:
                ...
                cursor.execute(
                    """
                    UPDATE stocks SET quantity = quantity - %s
                    WHERE user_id = %s AND stock_name = %s
                    """,
                ...
                cursor.execute(
                    """
                    DELETE FROM stocks WHERE user_id = %s AND stock_name = %s AND quantity <= 0
                    """,
                    (user_id, stock_name),
                )
